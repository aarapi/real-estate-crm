...
?>
<div id="resultDiv">
<div class="page-title"> <div class="title-env"> <h1 class="title"><?php echo __('Vendors');?> / <?php echo __('Staff');?></h1></div></div>
<div class="panel"><?php echo $this->Session->flash();?>
    <div class="panel-heading">
        <div class="btn-group">
            <?php $url=$this->Html->url(array('controller'=>'Vendors'));?>
	    <?php echo $this->Html->link('<span class="fa fa-plus"></span>&nbsp;'.__('Add New Vendors / Staff'),array('controller'=>'Vendors','action'=>'add'),array('escape'=>false,'class'=>'btn btn-success'));?>
...
<?php echo $this->Html->css('print');?>
    <div class="panel-body">
			    ...
			    <th><?php echo $this->Paginator->sort('contact', __('Mobile'), array('direction' => 'asc'));?></th>
			    <th><?php echo $this->Paginator->sort('address', __('Address'), array('direction' => 'asc'));?></th>
			    <th><?php echo $this->Paginator->sort('licence_no', __('Licence No.'), array('direction' => 'asc'));?></th>
                            <th><?php echo __('Action');?></th>
                        </tr>
                        <?php foreach ($Vendor as $post):
                        $id=$post['Vendor']['id'];?>
                        <tr>
                            <td><?php echo $this->Form->checkbox(false,array('value' => $post['Vendor']['id'],'name'=>'data[Vendor][id][]','id'=>"DeleteCheckbox$id",'class'=>'chkselect'));?></td>
                            <td><?php echo $serialNo++; ?></td>
                            <td><?php echo h($post['Vendor']['name']);?></td>
			    <td><?php echo h($post['Vendor']['contact']);?></td>
			    <td><?php echo h($post['Vendor']['address']);?></td>
			    <td><?php echo h($post['Vendor']['licence_no']);?></td>
