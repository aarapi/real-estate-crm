...
<div <?php if(!$isError){?>class="container"<?php }?>>
	...
	<div class="panel-heading"><?php echo __('Edit');?> <?php echo __('Deals Payments');?><?php if(!$isError){?><button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button><?php }?></div>            
		...
		<div class="form-group">
		    ...
		    <div class="col-sm-4">
		       <?php echo $this->Form->select("$k.DealsPayment.paymenttype_id",$paymentType,array('empty'=>__('Please Select'),'label'=>false,'class'=>'form-control')); ?>
		    </div>                                           
		</div>
		<div class="form-group">
			...
			<?php echo $this->Form->input("$k.DealsPayment.payment_date",array('type'=>'text','value'=>$this->Time->format($dtFormat,$post['DealsPayment']['payment_date']),'label' => false,'class'=>'form-control','placeholder'=>__('Payment Date'),'div'=>false));?>
			...
			<div class="col-sm-4">
			   <?php echo $this->Form->input("$k.DealsPayment.remarks", array('label'=>false,'placeholder'=>__('Transaction Reference'),'class'=>'form-control')); ?>
			</div>                                           
		    </div>
