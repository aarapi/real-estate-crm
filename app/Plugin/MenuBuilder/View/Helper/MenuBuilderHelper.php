 ...
 */
	public function build($id = null, $options = array(), &$data = null, &$isActive = false) {
		...
		if((isset($id) && ($id == 'children')))
		$wrapperFormat=$this->settings['wrapperFormat'];
		else
		$wrapperFormat='<ul%s>%s</ul>';
		return sprintf('%s'.$wrapperFormat."\n", $pad, $class, "\n".$out.$pad);
	}
 ...
 */
	protected function _buildItem(&$item, $pos = -1, &$isActive = false) {
		...
		if (isset($item['url']))
		{
			...
			{
				$check = (strpos(Router::normalize($controller), Router::normalize(strtolower($item['url']['controller']))));				
			}
			else
			{
				$check = Router::normalize($controller) === Router::normalize(strtolower($item['url']['controller']));
				if($check==false)
				{
					if(strpos($item['selName'],",")!== false)
					{
						$selArr=explode(",",$item['selName']);
						foreach($selArr as $selValue)
						{
							$check = Router::normalize($controller) === Router::normalize(strtolower($selValue));
							if($check)
							break;
						}
					}
					...
					{
						$check = Router::normalize($controller) === Router::normalize(strtolower($item['selName']));
					}
				}
			}
		}
		...
		if (is_null($item['url']['controller'])) {
			if($isActive)
			$noLinkFormat='<a class="collapsible-header waves-effect waves-cyan active" href="#">%s</a>';
			else
			$noLinkFormat=$this->settings['noLinkFormat'];
			$url = sprintf($noLinkFormat, $item['title']);
		} else {
